...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def prune(self, zs):
        ...
        self.weight = Parameter(self.weight.index_select(1,hidden_index).clone())
        return 
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def prune(self, hidden_index, intermediate_index=None):
        device = self.weight.device
        hidden_index = hidden_index.to(device)
        if intermediate_index is not None:
            intermediate_index = intermediate_index.to(device)
        
        assert torch.max(hidden_index,dim=0).values.item() < self.weight.shape[1]

        # import rpdb; rpdb.set_trace('0.0.0.0', 1024+torch.distributed.get_rank())

        if intermediate_index is not None:
            assert torch.max(intermediate_index,dim=0).values.item() < self.weight.shape[0]

        # self.weight -> [output_per_partition, hidden_size]
        self.weight = Parameter(self.weight.index_select(1,hidden_index).clone())
        if intermediate_index is not None:
            self.weight = Parameter(self.weight.index_select(0,intermediate_index).clone())

        if self.bias is not None:
            self.bias = Parameter(self.bias.index_select(0,intermediate_index).clone())
...
class RowParallelLinear(torch.nn.Module):
    ...
    def prune(self, hidden_index, intermediate_index):
        device = self.weight.device
        hidden_index = hidden_index.to(device)
        intermediate_index = intermediate_index.to(device)


        assert torch.max(hidden_index,dim=0).values.item() < self.weight.shape[0]
        assert torch.max(intermediate_index,dim=0).values.item() < self.weight.shape[1]

        self.weight = Parameter(self.weight.index_select(0,hidden_index).clone())
        self.weight = Parameter(self.weight.index_select(1,intermediate_index).clone())
        if self.bias is not None:
            self.bias = Parameter(self.bias.index_select(0,hidden_index).clone())
