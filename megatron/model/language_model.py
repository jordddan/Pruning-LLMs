...
class Embedding(MegatronModule):
    ...
    def prune(self, zs):
        assert self.word_embeddings.__class__ == tensor_parallel.VocabParallelEmbedding
        self.word_embeddings.prune(zs)
...
class TransformerLanguageModel(MegatronModule):
    ...
    def prune(self, zs=None):
        assert self.embedding.__class__ == Embedding
        self.embedding.prune(zs=zs)

        assert self.encoder.__class__ == ParallelTransformer
        self.encoder.prune(zs=zs)

        assert self.output_layer.__class__ == tensor_parallel.ColumnParallelLinear
        # import pdb
        # pdb.set_trace()
        self.output_layer.prune(zs["hidden_index"])
