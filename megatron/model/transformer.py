...
from megatron.model.utils import expand_each_element
...
class ParallelMLP(MegatronModule):
    ...
    def prune(self,zs,layer_num):
        ...
        args = get_args()
        new_intermediate_index = (intermediate_index + self.config.ffn_hidden_size // args.tensor_model_parallel_size).clone()
        expanded_intermediate_index = torch.cat([intermediate_index,new_intermediate_index],dim=-1)

        # import rpdb; rpdb.set_trace('0.0.0.0', 1024+torch.distributed.get_rank())

        assert self.dense_h_to_4h.__class__ == tensor_parallel.ColumnParallelLinear
        assert self.dense_4h_to_h.__class__ == tensor_parallel.RowParallelLinear
        self.dense_h_to_4h.prune(hidden_index,expanded_intermediate_index)
        self.dense_4h_to_h.prune(hidden_index,intermediate_index)
...
class ParallelAttention(MegatronModule):
    ...
    def prune(self, zs, layer_num):
        ...
        hidden_index = zs["hidden_index"]
        head_indexes = zs["head_indexes"]
        head_mask = zs["head_masks"][layer_num-1]

        if self.attention_type == AttnType.self_attn: # self attention prune
            # prune MHA qkv weight
            expand_size = self.config.kv_channels * 3
            head_mask_expand = expand_each_element(head_mask,expand_size)
            head_index_expand = torch.where(head_mask_expand==True)[0]
            self.query_key_value.prune(hidden_index,head_index_expand)
            # import pdb
            # pdb.set_trace()

        else: # cross attention prune

            # prune MHA q weight
            expand_size_q = self.config.kv_channels 
            head_mask_expand_q = expand_each_element(head_mask,expand_size_q)
            head_index_expand_q = torch.where(head_mask_expand_q==True)[0]
            self.query.prune(hidden_index,head_index_expand_q)

            # prune MHA kv weight 
            expand_size_kv = self.config.kv_channels * 2
            head_mask_expand_kv = expand_each_element(head_mask,expand_size_kv)
            head_index_expand_kv = torch.where(head_mask_expand_kv==True)[0]
            self.key_value.prune(hidden_index,head_index_expand_kv)

        expand_size_dense = self.config.kv_channels
        head_mask_expand_dense = expand_each_element(head_mask,expand_size_dense)
        head_index_expand_dense = torch.where(head_mask_expand_dense==True)[0]
        self.dense.prune(hidden_index,head_index_expand_dense)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def prune(self,zs):
        ...
        assert self.input_norm.__class__ == RMSNorm
        self.input_norm.prune(zs)      
        
        assert self.self_attention.__class__ == ParallelAttention
        self.self_attention.prune(zs,self.layer_number)

        assert self.post_attention_norm.__class__ == RMSNorm
        self.post_attention_norm.prune(zs)

        assert self.mlp.__class__ == ParallelMLP
        self.mlp.prune(zs,self.layer_number)
...
class ParallelTransformer(MegatronModule):
            ...
            with transformer_engine.pytorch.fp8_autocast(
                ...
                if self.recompute_granularity == 'full':
                    ...
                    for index in range(self.num_layers):
                        # if index in [5,8,11,14,17,20,23,26]:
                        #     continue
                        layer = self._get_layer(index)
    ...
    def prune(self,zs):
        ...
        # prune layernorm
        assert self.final_norm.__class__ == RMSNorm
        self.final_norm.prune(zs)

        # prune layers
        for layer in self.layers:
            assert layer.__class__ == ParallelTransformerLayer
            layer.prune(zs)
        
        args = get_args()
        if args.drop_layers is not None:
            drop_layers = [int(i) for i in args.drop_layers.split(",")]
